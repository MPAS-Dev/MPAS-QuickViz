...
def build_particle_file(fname_in, pointlon, pointlat, radius=150000., Nlen=20,
    ...
    case = 'CaseRelDiffVel_lon=%f_lat=%f_maxradius=%f_ts=%d_te=%d_deltat=%f_Nradius=%d_Nlen=%d/'%(pointlon, pointlat, radius,ts,te,deltat, Nradius, Nlen)
    print 'Case is ' + case
    check_folder(case)

    # allocate memory #{{{
    npart       = np.zeros((Nradius))
    K_rr        = np.zeros((Nradius,te-ts+1))
    K_xx        = np.zeros((Nradius,te-ts+1))
    K_xy        = np.zeros((Nradius,te-ts+1))
    K_yx        = np.zeros((Nradius,te-ts+1))
    K_yy        = np.zeros((Nradius,te-ts+1))
    #}}}
    ...
    for nlayer in layerrange: 
        ...
        for al in np.arange(Nradius):
            ...
            K_rr[al,:] = np.mean(np.mean(s*sr,axis=0),axis=1)
            K_xx[al,:] = np.mean(np.mean(u*sx,axis=0),axis=1)
            K_xy[al,:] = np.mean(np.mean(u*sy,axis=0),axis=1)
            K_yx[al,:] = np.mean(np.mean(v*sx,axis=0),axis=1)
            K_yy[al,:] = np.mean(np.mean(v*sy,axis=0),axis=1)
