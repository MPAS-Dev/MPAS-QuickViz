...
def build_particle_file(fname_in, pointlon, pointlat, radius=100000, Nlen=5, ts=25, deltat=2., nlayers=5, layerrange=np.arange(0,5), Ndim=4, degrees=False):  #{{{
    ...
    case = 'CaseBinningTensorPointIncr_lon=%f_lat=%f_r=%f_ts=%d_deltat=%f_Nlen=%d/'%(pointlon, pointlat, radius, ts, deltat, Nlen)
    ...
    for nlayer in layerrange: 
        ...
        for dtau in np.arange(Nlen):
            for ae in np.arange(Nec):
                ...
                lon1 = llon[particles,dtau ,ae]
                lat1 = llat[particles,dtau ,ae]
                lon2 = llon[particles,dtau+1,ae]
                lat2 = llat[particles,dtau+1,ae]
                # compute dispersion
