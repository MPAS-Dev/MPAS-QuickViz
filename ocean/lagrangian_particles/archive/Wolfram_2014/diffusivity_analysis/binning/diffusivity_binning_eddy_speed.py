...
def build_particle_file(fname_in, radius=100000, Nlen=5, ts=25, deltat=2., nlayers=5, layerrange=np.arange(0,5), degrees=False):  #{{{
    ...
    case = 'CaseBinningEnsembledEddySpeed_r=%f_ts=%d_deltat=%f_Nlen=%d_Nc=%d/'%(radius, ts, deltat, Nlen, Nc)
    ...
    for nlayer in layerrange: 
        ...
        for acluster in np.arange(Nc):
            ...
            meanu[acluster,:] = np.mean(velx[particles[acluster,:],:,:],axis=(0,2))
            meanv[acluster,:] = np.mean(vely[particles[acluster,:],:,:],axis=(0,2))
            eddyu[acluster,:] = np.std(velx[particles[acluster,:],:,:],axis=(0,2))
            eddyv[acluster,:] = np.std(vely[particles[acluster,:],:,:],axis=(0,2))
            ...
            #meanu[acluster,:] = np.mean(velx[particles[acluster,:],:,:])[np.newaxis]
            #meanv[acluster,:] = np.mean(vely[particles[acluster,:],:,:])[np.newaxis]
            #up = velx[particles[acluster,:],:,:] - meanu[acluster,:][np.newaxis,:,np.newaxis]
            #vp = vely[particles[acluster,:],:,:] - meanv[acluster,:][np.newaxis,:,np.newaxis]

            #eddyu[acluster,:] = np.sqrt(np.mean(up*up,axis=(0,2)))
            #eddyv[acluster,:] = np.sqrt(np.mean(vp*vp,axis=(0,2))
            
            # compute rho.  Note (Np,Nt,Ne) is dim of velx
            up = velx[particles[acluster,:],:,:] - meanu[acluster,:][np.newaxis,:,np.newaxis]
            vp = vely[particles[acluster,:],:,:] - meanv[acluster,:][np.newaxis,:,np.newaxis]
            ...
            #assert rho[acluster,0] == 1, 'rho not normalized correctly on %d!'%(acluster)

        np.savez(case+'layer%d'%(nlayer), ts=ts, te=ts+Nlen, deltat=deltat,
                eddyspeed=eddyspeed, meanspeed=meanspeed, meanu=meanu, rho=rho, rhou=rhou, rhov=rhov,
                meanv=meanv, eddyu=eddyu, eddyv=eddyv, npart=npart, x=x, y=y)
