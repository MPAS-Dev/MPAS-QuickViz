...
ds = xr.open_dataset(sys.argv[2] + '/365days_all_rlzn_particle_data_rads_rlzn0000.nc')
x = ds['lon'].values
y = ds['lat'].values
dt = ds['dtdays'].values
notoutcropped = ds['notoutcropped'].values
buoyancySurface = ds.Nb.values
tvec = np.hstack((np.array([0.0]), np.cumsum(ds['dtdays'][rlzn].values)))*86400.
...
effdiff = ed.EffectiveDiffusivity(np.squeeze(x[rlzn,:,alayer,:]),np.squeeze(y[rlzn,:,alayer,:]),
                    ...
                    kappa=kappa, calctype='ygrad', dt=12*60*60., dtout=1.*86400., dtreset=5.*86400,
                    saveloc=sys.argv[2] + '/effdiff/kappa%03d_production_eff_diff_conc_evolution_gradient_layer%04d/'%(kappa,alayer))
