...
def main():
  ...
  for afile in files:
    ...
    number = int(re.findall('[0-9].*[0-9]',afile)[0])
    procconnections = numberarray_between_strings(afile, 'ioProcNeighs after = ', 'ioProcNeighs after = Done')
    halos[number, procconnections ] = 1
  np.savez('io_proc_halos.npz',halos=halos)
