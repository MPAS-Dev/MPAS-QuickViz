...
load([char(dir) abc '_rpe_tau_Dlf.mat'])
...
loglog([1e-5 10000],meanDrpeDt(1)*[1 1],'r-')
hold on
loglog(tau_Dlf(2:8),meanDrpeDt(2:8),'bs-')

% data from c38g-y
%tau_Dlf = [.1 1 10 100 300 500 1000]
%meanDrpeDt = [1.1e-3 8.9e-4 5.6e-4 4.6e-4 9.6e-5 5.9e-5 2.8e-5]

tau_Dlf = [ 20 100 300 500 1000]
meanDrpeDt = [2.5e-3 4.6e-4 9.6e-5 5.9e-5 2.8e-5]
loglog(tau_Dlf,meanDrpeDt,'k*-')
...
% 'z-star','del2h=0','del2h=100','del2h=1000','del2h=100','del2h=1000'};
group_legend={...
 'z-level and z-star','z-tilde with z-level IC',...
    'z-tilde with isopycnal IC'};
%title('lock exchange, \nu_h=0.01')
...
%text(.005,.7,[ date ]);
%text(.005,0,[char(dir) abc]);
fig=[char(dir) abc(1:nsims) '_dRPEdt_tau_Dlf_isopyc2'];
print('-depsc2',['f/' fig '.eps']);
print('-djpeg',['f/' fig '.jpg']);
unix(['epstopdf f/' fig '.eps --outfile=f/' fig '.pdf']);
...
%%%%%%%%%%%%%%%%%%%% for revision 2, add isopycnal IC data:

% this never worked that well.

time_fields=[1:18];

dir={'c38'}; abc='opqrstu'; dims=1*[4 158];
nVertLevels = 20;
tau_Dlf = [1e3 1e2 10 1 .1 .01];

dir={'c38'}; abc='fghij'; dims=1*[4 158];
nVertLevels = 2;

dir={'c40'}; abc='fh'; dims=1*[4 158];
nVertLevels = 40;

dir={'c40'}; abc='jklmno'; dims=1*[4 158];
nVertLevels = 40;

dir={'c38'}; abc='e'; dims=1*[4 158];
nVertLevels = 2;
...
for j=1:length(abc)
%  [time,rpeTot,rpeNorm(:,j),DrpeDt,meanDrpeDt(j),keMeanTime(j),vertTransportMean(j),vertTransportZ(:,j)] ...
  [time,rpeTot,rpeNorm(:,j),DrpeDt,meanDrpeDt(j),keMeanTime(j),vertTransportMean(j),vertTransportZ] ...
    = sub_rpe(wd,dir,abc(j),netcdf_file, ...
    dims,time_fields,min_n,max_n,char(title_txt));
  fprintf(['meanDrpeDt ' char(dir) abc(j) ': %e \n'],meanDrpeDt(j));
end
