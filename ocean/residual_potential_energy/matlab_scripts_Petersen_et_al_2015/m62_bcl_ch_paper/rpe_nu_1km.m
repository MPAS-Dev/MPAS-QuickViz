...
time_fields=[1:7];
dir={'m62'}; abc='qnop'; dims=1*[160 576];
nu_h=[1 10 20 200];
grid_spacing=1e3;

netcdf_file = 'output.0000-01-01_00.00.00_50day_all.nc';
...
for j=1:length(abc)
  [time,rpeTot(:,j),rpeNorm(:,j),DrpeDt,meanDrpeDt(j),keMeanTime(j),vertTransportMean(j),vertTransportZ(:,j)] ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% RPE norm with time, many viscosities 1 km
...
grid on
title('MPAS-O Baroclinic channel, 1km, m62m-q')
xlabel('time, days')
ylabel('(RPE-RPE(0))/RPE(0)')
axis([0 300 -1e-7 1.4e-6])
...
%text(.005,.7,[ date ]);
%text(.005,0,[char(dir) abc]);
fig=[char(dir) abc(1:nsims) '_rpeTot_nu_1km'];
print('-depsc2',['f/' fig '.eps']);
print('-djpeg',['f/' fig '.jpg']);
unix(['epstopdf f/' fig '.eps --outfile=f/' fig '.pdf']);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% RPE with time, many viscosities 4 km
