...
filename = [wd char(dir) abc '/' char(hist_files)]
ncid = netcdf.open(filename,'nc_nowrite');
[dimname, nx] = netcdf.inqDim(ncid,0);
[dimname, ny] = netcdf.inqDim(ncid,1);
[dimname, nz] = netcdf.inqDim(ncid,2);
    TEMPfull = netcdf.getVar(ncid,netcdf.inqVarID(ncid,'TEMP'));
%    UVELfull = netcdf.getVar(ncid,netcdf.inqVarID(ncid,'UVEL'))/100.0; %m/s
%    VVELfull = netcdf.getVar(ncid,netcdf.inqVarID(ncid,'VVEL'))/100.0; %m/s
netcdf.close(ncid)
...
for nt=1:length(time_fields)
  fprintf('begin, nt=%g\n',nt)
  TEMP = squeeze(TEMPfull(:,:,:,time_fields(nt)));
%    TEMP = squeeze(TEMPall(time_fields(nt),:,:,:))
%    TEMP = squeeze(TEMPall(time_fields(nt),:,:,:))
...
% time in seconds, convert from hours
time = (time_fields-1)*60*60
rpeTot
