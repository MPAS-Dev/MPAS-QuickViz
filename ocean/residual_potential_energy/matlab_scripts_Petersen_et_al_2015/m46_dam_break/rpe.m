...
min_n=102; % time 16:50
max_n = 104; % time 17:10
...
grid on
set(gca,'xTick',10.^[0:5],'xMinorGrid','off')
get(gca)
...
axis([0 17 -1e-5 8e-5])
%axis tight
group_legend={'\nu_h=0.01, Re_{{\Delta}x}=2.5e4','\nu_h=0.1, Re_{{\Delta}x}=2.5e3','\nu_h=1, Re_{{\Delta}x}=2.5e2','\nu_h=10, Re_{{\Delta}x}=25','\nu_h=100, Re_{{\Delta}x}=2.5','\nu_h=200, Re_{{\Delta}x}=1.2',...
	      };
...
%axis tight
group_legend={...
 'MPAS-O z-star','MITGCM','MOM','ROMS','MPAS-O isopycnal.'};
%title('lock exchange, \nu_h=0.01')
legend(group_legend,'location','NorthWest')
set(gcf,'PaperPositionMode','auto','color',[.8 1 .8], ...
	'PaperPosition',[0 0 4 3])
