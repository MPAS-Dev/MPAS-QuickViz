#Generates the Drake Passage fields on the 0.5x0.5 rectangular grid
...
lon_index	    = (np.abs(lon - lon_section)).argmin()
...
layer	= layer[:, lat_min_index:lat_max_index, lon_index]
	
	
dy	= 6371000 * 2 * np.pi * 0.5 / 360 + np.zeros(len(lat))	
...
for year_i in range(int(np.min(time)), int(np.min(time))+len(time_year)):
	...
	for file_i in range(len(files_month)):
		...
		salt	= fh.variables['timeMonthly_avg_activeTracers_salinity'][0, :, lat_min_index:lat_max_index, lon_index] 	#Salinity (g/kg)
		temp	= fh.variables['timeMonthly_avg_activeTracers_temperature'][0, :, lat_min_index:lat_max_index, lon_index] 	#Temperature (deg C)
		u_vel	= fh.variables['timeMonthly_avg_velocityZonal'][0, :, lat_min_index:lat_max_index, lon_index] 	#Zonal velocity (m/s)
		...
		#Get the zonal mean
		salt_depth[file_i]	= salt
		temp_depth[file_i]	= temp
		u_vel_depth[file_i]	= u_vel
		dens_depth[file_i]	= dens		
	#------------------------------------------------------------------------------
	...
	filename 	= directory+'Data/Drake_Passage/E3SM_data_year_'+str(year_i).zfill(4)+'.nc'
	...
	fh.createVariable('layer', float, ('depth', 'lat'), zlib=True)
	fh.createVariable('DY', float, ('lat'), zlib=True)
	...
	fh.variables['DY'].longname	       = 'y-spacing'
	fh.variables['SALT'].longname	       = 'Salinity'
	fh.variables['TEMP'].longname	       = 'Potential temperature'
	fh.variables['UVEL'].longname	       = 'Zonal velocity'
	fh.variables['POT_DENS'].longname	   = 'Potential density'
	  
	fh.variables['depth'].units 		= 'm'
	fh.variables['lat'].units 	        = 'degrees N'
	fh.variables['layer'].units 		= 'm'
	fh.variables['DY'].units                = 'm'
