...
def parse_plot_args(ds_in, months, sigmabins, clims):
    """
    """
    ...
    meshes = list(ds_in)
    timestrings = list(ds_in[meshes[0]])[::-1]
...
def plot_2Dpanel(
    ...
    # Plot sigma contours and add dummy plot for legend
    if plot_sigma:
...
def plot_2Dvariable(ds_in, varname, units, scale=1, months=None, sigmabins=None, clims=None, cmap=None):
    ...
    for row, tstr in zip(axs, tstrs):
        ...
        for ax, mesh in zip(row, meshes):
            ...
            # Plot sliced dataset
            c1, variable = plot_2Dpanel(ax, ds, varname, scale=scale, clim=clims[0], cmap=cmap, plot_sigma=True)
            res.append(variable)
