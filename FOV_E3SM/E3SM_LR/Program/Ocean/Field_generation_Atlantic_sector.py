...
for year_i in range(int(np.min(time)), int(np.min(time))+len(time_year)):
	...
	salt_depth_year	= np.sum(salt_depth * month_days_all, axis = 0)
	temp_depth_year	= np.sum(temp_depth * month_days_all, axis = 0)
	u_vel_depth_year= np.sum(u_vel_depth * month_days_all, axis = 0)
	dens_depth_year	= np.sum(dens_depth * month_days_all, axis = 0)			
	#-----------------------------------------------------------------------------------------
	...
	fh.createVariable('SALT', float, ('month', 'depth', 'lat'), zlib=True)
	fh.createVariable('TEMP', float, ('month', 'depth', 'lat'), zlib=True)
	fh.createVariable('UVEL', float, ('month', 'depth', 'lat'), zlib=True)
	fh.createVariable('POT_DENS', float, ('month', 'depth', 'lat'), zlib=True)
	fh.createVariable('SALT_mean', float, ('depth', 'lat'), zlib=True)
	fh.createVariable('TEMP_mean', float, ('depth', 'lat'), zlib=True)
	fh.createVariable('UVEL_mean', float, ('depth', 'lat'), zlib=True)
	fh.createVariable('POT_DENS_mean', float, ('depth', 'lat'), zlib=True)
	...
	fh.variables['SALT_mean'].longname	       = 'Zonally-averaged salinity (yearly mean)'
	fh.variables['TEMP_mean'].longname	       = 'Zonally-averaged potential temperature (yearly mean)'
	fh.variables['UVEL_mean'].longname	       = 'Zonally-averaged zonal velocity (yearly mean)'
	fh.variables['POT_DENS_mean'].longname	   = 'Zonally-averaged potential density (yearly mean)'
	...
	fh.variables['SALT_mean'].units        = 'g/kg'
	fh.variables['TEMP_mean'].units        = 'deg C'
	fh.variables['UVEL_mean'].units        = 'm/s'
	fh.variables['POT_DENS_mean'].units    = 'kg/m^3'
	...
	fh.variables['SALT_mean'][:]        = salt_depth_year
	fh.variables['TEMP_mean'][:]        = temp_depth_year
	fh.variables['UVEL_mean'][:]        = u_vel_depth_year
	fh.variables['POT_DENS_mean'][:]    = dens_depth_year
